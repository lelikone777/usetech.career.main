...
function fontsStyleLess(cb) {
    let file_content = fs.readFileSync(source_folder + "/less/_fonts.less");
    if (file_content == "") {
        fs.writeFile(source_folder + "/less/_fonts.less", "", cb);
        return fs.readdir(path.build.fonts, function (err, items) {
            if (items) {
                let c_fontname;
                for (var i = 0; i < items.length; i++) {
                    ...
                    if (c_fontname != fontname) {
                        fs.appendFile(
                            source_folder + "/less/_fonts.less",
                            '@include font("' +
                            fontname +
                            '", "' +
                            fontname +
